<h1 align="center" style="color:#00FFFF;">👋 Hey, I'm Narayan Anantha Krishnan</h1>

<p align="center">
🤖 <b>AI Engineer</b> | 💻 <b>Software Developer</b> | 🔬 <b>Researcher</b><br>
🚀 Building agentic AI systems, full-stack apps, and everything in between
</p>

---

## 🚀 About Me

🎓 **M.S. in Computer Science** @ Syracuse University (May 2026) — GPA 3.78  
🔬 **Graduate Research Assistant** under Prof. Senanayake Kumarawadu  
📝 **2 peer-reviewed papers** accepted at **FLAIRS-39 (2026)** — agentic RAG & edge LLM deployment  
🏆 **Judges' Favorite Award** @ ECS Research Day Hackathon (2025) | **Head of CSI** (25+ member team)  
🏐 Intramural volleyball champion @ Syracuse | Former state-level aerobic gymnast  
🌟 Open to **AI Engineer / SWE / Analyst** roles — F-1 OPT + STEM extension eligible

---

## 📝 Publications

📄 **[An Iterative Self-Correcting Agentic RAG System](https://journals.flvc.org/FLAIRS/article/view/141838)** — FLAIRS-39, 2026 *(Full paper)*  
📄 **[Automated IoT Threat Monitoring & Mitigation using Tiny LLMs](https://journals.flvc.org/FLAIRS/article/view/141840)** — FLAIRS-39, 2026 *(Short paper)*

---

## 📌 Featured Projects

### 🤖 [Swaylytics — AI-Powered Autonomous Data Analyst](https://www.loom.com/share/2ddf284bccb44e9da42b5b5c5bcd61ad) <sub>*(Loom demo)*</sub>
A multi-agent system that automates end-to-end exploratory data analysis on user-uploaded datasets.
- **LoRA-fine-tuned DeepAnalyze-8B** worker + **Gemini 3 Flash** orchestration via **LangGraph**  
- Outperformed **GPT-4-turbo by ~15%** on the **DS-1000** benchmark with a smaller 8B model  
- 4 specialized agents — *planner, analyzer, visualizer, critic* — with iterative self-correction  
- Stack: PyTorch, FastAPI, Docker, LangGraph, LoRA

### 🧠 Agentic RAG System with Iterative Self-Correction
An intelligent knowledge retrieval framework — work behind my FLAIRS-39 full paper.
- Multi-agent architecture: retrieval, reasoning, planning, and verification agents  
- **92% routing accuracy** on complex multi-hop queries  
- **Pre-Act planning** + automated question decomposition → 35% fewer incomplete responses  
- Stack: LangChain, LangGraph, ChromaDB, OpenAI Embeddings, FastAPI

### 🛡️ Edge IoT Threat Detection with Tiny LLMs
Fine-tuned 3 SLMs for on-device security classification — work behind my FLAIRS-39 short paper.
- **23.37 pp accuracy improvement** over XGBoost baseline (76.93% vs 53.56%)  
- **129ms inference latency** at 14.25 tokens/sec on edge hardware  
- Automated threat mitigation pipeline mapping to **MITRE CAPEC** strategies  
- Stack: PyTorch, Hugging Face Transformers, LoRA/PEFT

### 🌍 Panvel Municipal Corporation — Solid Waste Management Portal
Civic platform serving **5,000+ daily users** across desktop and mobile.
- Real-time data sync via RESTful APIs → 40% reduction in reporting delays  
- 29% page load time improvement through code-splitting, lazy loading, caching  
- Dockerized + CI/CD on GitHub Actions + Jenkins  
- Stack: React.js, Node.js, AWS, Docker

---

## 🛠️ Tech Stack

### **Languages**
![Python](https://img.shields.io/badge/-Python-000?style=for-the-badge&logo=python&logoColor=FFD700)
![JavaScript](https://img.shields.io/badge/-JavaScript-000?style=for-the-badge&logo=javascript&logoColor=FFD700)
![TypeScript](https://img.shields.io/badge/-TypeScript-000?style=for-the-badge&logo=typescript&logoColor=3178C6)
![Java](https://img.shields.io/badge/-Java-000?style=for-the-badge&logo=openjdk&logoColor=FF8C00)
![C++](https://img.shields.io/badge/-C/C++-000?style=for-the-badge&logo=cplusplus&logoColor=00FFFF)
![SQL](https://img.shields.io/badge/-SQL-000?style=for-the-badge&logo=mysql&logoColor=FF4500)
![HTML5](https://img.shields.io/badge/-HTML5-000?style=for-the-badge&logo=html5&logoColor=FF4500)
![CSS3](https://img.shields.io/badge/-CSS3-000?style=for-the-badge&logo=css3&logoColor=00FFFF)
![Haskell](https://img.shields.io/badge/-Haskell-000?style=for-the-badge&logo=haskell&logoColor=5D4F85)
![Bash](https://img.shields.io/badge/-Bash-000?style=for-the-badge&logo=gnubash&logoColor=4EAA25)

### **AI / ML / GenAI**
![PyTorch](https://img.shields.io/badge/-PyTorch-000?style=for-the-badge&logo=pytorch&logoColor=EE4C2C)
![TensorFlow](https://img.shields.io/badge/-TensorFlow-000?style=for-the-badge&logo=tensorflow&logoColor=FF6F00)
![HuggingFace](https://img.shields.io/badge/-HuggingFace-000?style=for-the-badge&logo=huggingface&logoColor=FFD21E)
![scikit-learn](https://img.shields.io/badge/-scikit--learn-000?style=for-the-badge&logo=scikitlearn&logoColor=F7931E)
![Pandas](https://img.shields.io/badge/-Pandas-000?style=for-the-badge&logo=pandas&logoColor=150458)
![NumPy](https://img.shields.io/badge/-NumPy-000?style=for-the-badge&logo=numpy&logoColor=013243)
![LangChain](https://img.shields.io/badge/-LangChain-000?style=for-the-badge&logoColor=00FFFF)
![LangGraph](https://img.shields.io/badge/-LangGraph-000?style=for-the-badge&logoColor=FF00FF)
![LangSmith](https://img.shields.io/badge/-LangSmith-000?style=for-the-badge&logoColor=FFD700)
![LlamaIndex](https://img.shields.io/badge/-LlamaIndex-000?style=for-the-badge&logoColor=00BFFF)

**LLMs I've worked with:** GPT-4 · Claude · Gemini · Llama · Qwen · Phi · Gemma  
**Concepts:** RAG · LoRA/PEFT · Prompt Engineering · Multi-Agent Systems · vLLM · BERT · NLP

### **Frontend & Backend**
![React](https://img.shields.io/badge/-React-000?style=for-the-badge&logo=react&logoColor=00FFFF)
![Next.js](https://img.shields.io/badge/-Next.js-000?style=for-the-badge&logo=nextdotjs&logoColor=FFFFFF)
![TailwindCSS](https://img.shields.io/badge/-TailwindCSS-000?style=for-the-badge&logo=tailwindcss&logoColor=00FFFF)
![Node.js](https://img.shields.io/badge/-Node.js-000?style=for-the-badge&logo=node.js&logoColor=32CD32)
![Express](https://img.shields.io/badge/-Express-000?style=for-the-badge&logo=express&logoColor=FFFFFF)
![FastAPI](https://img.shields.io/badge/-FastAPI-000?style=for-the-badge&logo=fastapi&logoColor=009688)
![Flask](https://img.shields.io/badge/-Flask-000?style=for-the-badge&logo=flask&logoColor=FFFFFF)

### **Cloud, DevOps & Databases**
![AWS](https://img.shields.io/badge/-AWS-000?style=for-the-badge&logo=amazonaws&logoColor=FF8C00)
![GCP](https://img.shields.io/badge/-GCP-000?style=for-the-badge&logo=googlecloud&logoColor=4285F4)
![Docker](https://img.shields.io/badge/-Docker-000?style=for-the-badge&logo=docker&logoColor=00BFFF)
![Kubernetes](https://img.shields.io/badge/-Kubernetes-000?style=for-the-badge&logo=kubernetes&logoColor=326CE5)
![Jenkins](https://img.shields.io/badge/-Jenkins-000?style=for-the-badge&logo=jenkins&logoColor=D24939)
![GitHub Actions](https://img.shields.io/badge/-GitHub_Actions-000?style=for-the-badge&logo=githubactions&logoColor=2088FF)
![Git](https://img.shields.io/badge/-Git-000?style=for-the-badge&logo=git&logoColor=FF4500)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-000?style=for-the-badge&logo=postgresql&logoColor=336791)
![MongoDB](https://img.shields.io/badge/-MongoDB-000?style=for-the-badge&logo=mongodb&logoColor=32CD32)
![MySQL](https://img.shields.io/badge/-MySQL-000?style=for-the-badge&logo=mysql&logoColor=4479A1)
![ChromaDB](https://img.shields.io/badge/-ChromaDB-000?style=for-the-badge&logoColor=FF00FF)

### **Dev Tools & Analytics**
![Claude Code](https://img.shields.io/badge/-Claude_Code-000?style=for-the-badge&logo=anthropic&logoColor=D97757)
![Cursor](https://img.shields.io/badge/-Cursor-000?style=for-the-badge&logoColor=FFFFFF)
![Copilot](https://img.shields.io/badge/-GitHub_Copilot-000?style=for-the-badge&logo=githubcopilot&logoColor=FFFFFF)
![Jupyter](https://img.shields.io/badge/-Jupyter-000?style=for-the-badge&logo=jupyter&logoColor=F37626)
![Tableau](https://img.shields.io/badge/-Tableau-000?style=for-the-badge&logo=tableau&logoColor=FF8C00)
![PowerBI](https://img.shields.io/badge/-PowerBI-000?style=for-the-badge&logo=powerbi&logoColor=FFD700)

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=NarayanAnanthaKrishnan&show_icons=true&theme=highcontrast" alt="GitHub Stats">
  <br>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=NarayanAnanthaKrishnan&theme=highcontrast" alt="GitHub Streak">
  <br>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=NarayanAnanthaKrishnan&layout=compact&theme=highcontrast" alt="Top Languages">
</p>

---

## 📫 Let's Connect

🔗 **LinkedIn:** [linkedin.com/in/narayan-anantha-krishnan](https://linkedin.com/in/narayan-anantha-krishnan)  
📧 **Email:** [narayanananthakrishnann@gmail.com](mailto:narayanananthakrishnann@gmail.com)  
🌍 **Portfolio:** *Coming soon*

---

<p align="center">
  <i>"Build, measure, ship — repeat."</i>
</p>
